...
#    Flow(init1+'sigtemp'+'%d'%(i+1),[sig1,sig2],'cat axis=2 ${SOURCES[1]}')
#    Flow(init1+'shottimetemp'+'%d'%(i+1),[shottime1,shottime2],'cat axis=2 ${SOURCES[1]}')
#    Flow(init1+'blendedtemp'+'%d'%(i+1),[init1+'sigtemp'+'%d'%(i+1),init1+'shottimetemp'+'%d'%(i+1)],
#    	'''
#    	blend shot_time_in=${SOURCES[1]} shot_time_out=${SOURCES[1]} 
#    	''')
#    	
#    Flow(init1+"%d"%(i+1),[init1+'blendedtemp'+'%d'%(i+1),blended1,sig1], 
#         '''
#         window n2=%d | add scale=-1,1 ${SOURCES[1]} | add scale=%g,%g ${SOURCES[2]}
#         '''%(n2,fraction,1))
#    Flow(init2+"%d"%(i+1),[init1+'blendedtemp'+'%d'%(i+1),blended2,sig2], 
#         '''
#         window n2=%d f2=%d | add scale=-1,1 ${SOURCES[1]} | add scale=%g,%g ${SOURCES[2]}
#         '''%(n2,n2-1,fraction,1))
...
def deblendfft(unblended1,
    ...
    Flow(init1+'-snrsa',snrsa,'cat axis=2 ${SOURCES[1:%d]}|math output="10*log(input)/log(10)" | transp | put n1=%d o1=1 d1=1 '%(len(snrsa),len(snrsa)))
    Flow(init2+'-snrsb',snrsb,'cat axis=2 ${SOURCES[1:%d]}|math output="10*log(input)/log(10)" | transp |  put n1=%d o1=1 d1=1'%(len(snrsb),len(snrsb)))

    Graph(init1+'-snrsa','title="Data error 1" label1=Iteration label2="c(n)" symbosz=5 symbol="o" min1=0 max1=%d d1=1'%niter)	
    Graph(init2+'-snrsb','title="Data error 2" label1=Iteration label2="c(n)" symbosz=5 symbol="o" min1=0 max1=%d d1=1'%niter)		
...
#######################################################################################
    	if i % ddip ==0 :
		dip1=init1+'dip%d'%int(i/ddip)
		dip2=init2+'udip%d'%int(i/ddip)
		Flow(dip1,[sig1,'mask'],
     			'''
...
#######################################################################################
    ...
    Flow(init1+'-snrsa',snrsa,'cat axis=2 ${SOURCES[1:%d]}|math output="10*log(input)/log(10)" | transp | put n1=%d o1=1 d1=1 '%(len(snrsa),len(snrsb)))
    Flow(init2+'-snrsb',snrsb,'cat axis=2 ${SOURCES[1:%d]}|math output="10*log(input)/log(10)" | transp |  put n1=%d o1=1 d1=1'%(len(snrsb),len(snrsb)))
