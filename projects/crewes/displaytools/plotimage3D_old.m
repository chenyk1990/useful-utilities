...
%
% Seismic data stored in a 3D matrix are presented in a way that facilites
% easy viewing. Data can be viewed as either inline (iline) or crossline
% (xline) vertical sections or as time slices. The views presented are
% always 2D panels from the 3D volume and are presented as images (i.e. not
% wiggle traces). Controls are provides to adjust cliping and tostep
% through the volume sequentially. Additionally, when more than one
% plotimage3D window is active, then they can be linked and browesed
% simultaneously.
%
...
% NOTE2: This function is designed to work with the specific size window
% that it creates by default. If you resize this window significantly, then
% the axes labels (coordinates) may become incorrect. There is currently no
% workaround for this.
...
% hpreviousbutton ... 'previous' ... not used
