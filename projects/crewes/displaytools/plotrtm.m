function ha=plotrtm(recsnaps,sousnaps,refsnaps,vel,xs,zs,xlabels,zlabels,titles,vtitle,cmap,secondarytitle)
...
if(strcmp(action,'init'))
    ...
    ngath=length(recsnaps);
    if(iscell(zs))
        if(length(zs)~=ngath)
            error('if a cell array, length(ys) must equal length(recsnaps)');
        end
    else
        tmp=zs;
        zs=cell(1,ngath+1);
    ...
    if(iscell(xs))
        if(length(xs)~=ngath)
            error('if a cell array, length(xs) must equal length(recsnaps)');
        end
    else
        tmp=xs;
        xs=cell(1,ngath+1);
    ...
    if(iscell(zlabels))
        if(length(zlabels)~=ngath)
            error('if a cell array, length(ylabels) must equal length(recsnaps)');
        end
    else
        tmp=zlabels;
    ...
    if(iscell(xlabels))
        if(length(xlabels)~=ngath)
            error('if a cell array, length(xlabels) must equal length(recsnaps)');
        end
    else
        tmp=xlabels;
    ...
    if(length(titles)~=ngath)
        error('if a cell array, length(titles) must equal length(recsnaps)');
    end
    
    %insert the velocity model as the first snapshot
    recsnaps{ngath+1}=recsnaps{ngath};%this grows the snapshots by one
    sousnaps{ngath+1}=recsnaps{ngath};%this grows the snapshots by one
    refsnaps{ngath+1}=recsnaps{ngath};%this grows the snapshots by one
    titles{ngath+1}=titles{ngath};
    for k=ngath-1:-1:1
        recsnaps{k+1}=recsnaps{k};
        sousnaps{k+1}=sousnaps{k};
        refsnaps{k+1}=refsnaps{k};
        titles{k+1}=titles{k};
    end
    recsnaps{1}=vel;
    sousnaps{1}=vel;
    ...
    numbers=[recsnaps{imin}(:);recsnaps{imax}(:)];%use these two snaps for statistics
    aveamp=mean(numbers);
    stddev=std(numbers);
    ...
    for k=1:ngath+1
        ...
        if(iscomplex(recsnaps{k}))
            error('complex input not supported in plotsnaps')
        end
        if(k==1)
            gath=recsnaps{k};
        else
            gath=recsnaps{k}+1i*recsnaps{1};
        end
