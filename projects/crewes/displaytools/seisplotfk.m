function datar=seisplotfk(seis,t,x,dname,fmax,gridy,gridx)
% SEISPLOTFK: plots a seismic gather and its fk spectrum side-by-side
%
% datar=seisplotfk(seis,t,x,dname,fmax,gridy,gridx)
%
...
% ************ default = .25/(t(2)-t(1)) which is half of Nyquist ***********
...
%     end
%     if(ytemporal==1)
%         title({dname ,['f-k space, knyq=' num2str(knyq) ', fnyq=' num2str(fnyq)]});
%     else
%         title({dname ,['kx-ky space, kxnyq=' num2str(knyq) ', kynyq=' num2str(fnyq)]});
%     end
% 
%     if(max(x)<maxmeters)
%         xlabel('wavenumber (m^{-1})');
