function seisplotcomp(seis,t,x,sref,xevery,xstart,xend,xtent,kolor,dnamea,dnameref)
% SEISPLOTCOMP ... create a sesimic image plot (in a new figure window) with an overlay of comparison traces
%
% seisplotcomp(seis,t,x,sref,xevery,xstart,xend,xtent,kolor,dname)
%
...
% seismic sections in a sinlge window for comparison.
%
...
% sref ... matrix identically sized to seis containing the comparison traces
% xevery ... overlay a comparison trace every this many x units
% xstart ... plot the first comparison trace at this x coordinate
% xend ... plot the last comparison trace at this x coordinate
% xtent ... the maximum amplitude on any comparison trace will get this deflection (in x coordinate units)
% dname ... dataset name (string). Used to title the plot and to label the figure
% dnameref ... name of the comparison dataset
% 
...
if(strcmp(action,'init'))
    ...
    dname=[dnamea ' with ' dnameref ' overlay '];
    xname='';
    yname='';
    x=double(x);
...
%     brighten(.5);
    ...
    istart=near(x,xstart);
    iend=near(x,xend);
    dx=abs(x(2)-x(1));
    ievery=round(xevery/dx);
    iplot=istart(1):ievery:iend(1);
    amaxref=max(abs(sref(:)));
    href=zeros(size(iplot));
    for k=1:length(iplot)
       href(k)=line(x(iplot(k))+sref(:,iplot(k))*xtent/amaxref,t,'color',kolor,'linewidth',.25); 
    end
    ...
    %make a clip control
    xnow=.91;ynow=.875;wid=.055;ht=.05;sep=.005;
