...
%  Name        ... value
% 'distribute' ... 'y' or 'n' . This determines if the signals are on top of each other ('n') or
%           distributed sot that each one is by itself ('y').
% '*********** default is 'y' ***************
% 'direction' ... 'h' means horizontal 'v' means vertical
%  *********** default is 'h' *********
% 'overlay' ... 'y' or 'n'. If 'y', the the first signal is taken as a reference signal. All other 
%           signals are then plotted with the first signal "beneath" them in black. In this mode all
...
if(nargin>2)
    ...
    for k=1:2:nargs
        ...
        if(strcmp(arg,'overlay'))
            overlay=varargin{k+1};
            if(~strcmp(overlay,'y')&&~strcmp(overlay,'n'))
                error('''overlay'' must be either ''y'' or ''n''')
            end
            if(strcmp(overlay,'y'))
                overlay=1;
            else
...
if(overlay==1)
    ...
    iother=2:ntraces;
    traces=traces(iother);
    times=times(iother);
    linestyles=linestyles(iother);
    markers=markers(iother);
    names=names(iother);
    colors=colors(iother);
    ntraces=ntraces-1;
end
...
for k=1:ntraces
    ...
    if(~overlay)
        ...
        hh=zeros(1,3);%for trace handles
    end
    ...
    if(rsmpl>1)
        ...
        if(overlay==1 && k==1)
            tref2=linspace(tref(1),tref(end),rsmpl*length(tref));
            trref=interpbl(tref,trref,tref2);
            tref=tref2;
        end
    end
    %nameshift=-.2;
    if(strcmp(direction,'h'))
        if(~overlay)
            ...
            if(~strcmp(ls,'none'))
                hhtmp=plot(tref,trref+ystart+(k-1)*inc,colref,time,trace+ystart+(k-1)*inc);
                zd=zorder(k)*ones(size(time));
                hh(1)=hhtmp(2);hh(3)=hhtmp(1);
                set(hh(1),'linestyle',ls,'zdata',zd);
                set(hh(3),'linestyle',lstyleref,'marker',mrkref);
            end
            if(~strcmp(mk,'none'))
                hhtmp=plot(tref,trref+ystart+(k-1)*inc,colref,time(1:ievery(k):end),trace(1:ievery(k):end)+ystart+(k-1)*inc);
                zd=zorder(k)*ones(size(time(1:ievery(k):end)));
                hh(2)=hhtmp(2);hh(3)=hhtmp(1);
                set(hh(2),'linestyle','none','marker',mk,'zdata',zd);
                set(hh(3),'linestyle',lstyleref,'marker',mrkref);
            end
            if(zerolines)
        ...
        if(~overlay)
            ...
            if(~strcmp(ls,'none'))
                hhtmp=plot(trref+ystart+(k-1)*inc,tref,colref,trace+ystart+(k-1)*inc,time);
                zd=zorder(k)*ones(size(trace));
                hh(1)=hhtmp(2);hh(3)=hhtmp(1);
                set(hh(1),'linestyle',ls,'zdata',zd);
                set(hh(3),'linestyle',lstyleref,'marker',mrkref);
            ...
            if(~strcmp(mk,'none'))
                hhtmp=plot(trref+ystart+(k-1)*inc,tref,colref,trace(1:ievery(k):end)+ystart+(k-1)*inc,time(1:ievery(k):end));
                zd=zorder(k)*ones(size(trace(1:ievery(k):end)));
                hh(1)=hhtmp(2);hh(3)=hhtmp(1);
                set(hh(2),'linestyle','none','marker',mk,'zdata',zd);
                set(hh(3),'linestyle',lstyleref,'marker',mrkref);
