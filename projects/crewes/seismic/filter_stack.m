...
%
% This is just a simple interface to filtering programs suitable for 2D or 3D matrices.
% 
...
% ************ default 'butterband' ************
% NOTE: filtf and butterband are separate filter programs. See their help for more info.
...
if(threeD)
    ...
    if(isa(stack,'single'))
        stackf=single(zeros(nt,nxlines,nilines));
    else
        ...
        for ki=1:nilines
            if(isa(stack,'single'))
                ...
                if(sum(abs(tmp))>small)%avoid filtering a zero trace
                    switch method
                        case 'filtf'
                            tmp2=filtf(tmp,t,fmin,fmax,phase);
                            stackf(:,kx,ki)=single(tmp2);
                        case 'butterband'
                            tmp2=butterband(tmp,t,flow,fhigh,norder,phase);
                            stackf(:,kx,ki)=single(tmp2);
                    end
                end
            else
    ...
    if(isa(stack,'single'))
        stackf=single(zeros(size(stack)));
    else
