function [wavelets,tws]=extract_wavelets_matchT(s,t,r,t0s,twins,wlen,mu,pctnotcausal)
...
%          results.
% *********** default = 0.2 ************
% mu ... tradeoff parameters between wavelet smoothness and data fitting.
%          Two elements, first is for smoothness, second for time.
% *********** default mu=[1 1] ************
...
%     rg=r(ind).*mwindow(length(ind));
    ...
    [w,tws{k}]=matchT(rg,sg,t(ind),mlen,icausal,mu);
    if(icausal>=1)
        %sg1=convm(r(ind),w);
        izero=near(tws{k},0);
