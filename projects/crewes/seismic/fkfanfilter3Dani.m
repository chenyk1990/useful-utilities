function [seisf,masks,kx,ky]=fkfanfilter3Dani(seis,t,x,y,vax,vay,dvx,dvy,tpad,xpad,ypad,fmask)
...
%
% FKFANFILTER3D designs and applies an f-k (frequency-wavenumber) fan
% reject filter in 3D. The reject region is fan-shaped (when viewed in
% frequency and radial wavenumber) and defined by two bounding apparent
% velocities, va1 and va2. These are expressed as positive numbers and the
% filter is rotationally invariant about the vertical (t) axis. A raised
% cosine taper is applied to the filter edge. The filter is applied to each
% frequency as a radially symmetric multiplier (or mask) whose values lie
% between 0 and 1. This mask can be examined by returning all four values
