function [wavelets,tws]=extract_wavelets_roywhite(s,t,r,t0s,twins,wlen,mu,stab,fsmo,method,pctnotcausal)
% EXTRACT_WAVELETS_ROYWHITE: extract time variant embedded wavelets with Roy White's method
%
% [wavelets,tws]=extract_wavelets_roywhite(s,t,r,t0s,twins,wlen,mu,stab,fsmo,method,pctnotcausal)
%
...
for k=1:nwaves
    ...
    t2=t0s(k)+.5*twins(k);
    %mlen=wlen*twins(k);
    [wavelets{k},tws{k}]=waveest_roywhite(s,t,r,t,t1,t2,wlen,stab,mu,fsmo,method,pctnotcausal);
end
% for k=1:nwaves
%     %build gaussian
%     sig=twins(k)*sigma;
