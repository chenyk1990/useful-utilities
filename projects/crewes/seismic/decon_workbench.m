...
global DRAGLINE_MOTION DRAGLINE_YLIMS DRAGLINE_SHOWPOSN DRAGLINE_CALLBACK DRAGLINE_MOTIONCALLBACK DRAGLINE_PAIRED DRAGLINE_CC
...
if(strcmp(action,'init'))
   ...
   %check input args for consistency 
   [nt,ntr]=size(seis);
   if(length(t)~=nt)
   ...
   if(length(tref)~=ntr)
       error('tref and sref have incompatible sizes')
   end
   if(size(tref,1)~=ntr)
       tref=tref';
   end
   if(length(xref)~=1)
       error('xref must be a scalar');
   end
   xrange=max(x)-min(x);
   if(xref<(min(x)-xrange))
       error('xref is not near the seismic x coordinates');
   end
   if(xref>max(x)+xrange)
       error('xref is not near the seismic x coordinates');
   end
   ...
   if(~isempty(name))
      set(gcf,'name',['Decon Workbench:' name]);
...
%     end
    ...
    switch thisalg
        ...
        case 'deconw'
            ...
            stab=findparm('stab',parmlist);
            seisd=deconw_stack(seis,t,0,tstart,tend,1,top,stab);
            thisresult.seis=seisd;
            thisresult.seisf=[];
            if(fapply==1)
                seisd=butterband(seisd,t,flow,fhigh,norder,filtphase);
...
elseif(strcmp(action,'waveex'))
    ...
    if(isempty(thisresult))
        ...
        s=seis(:,is1);
        t=udat{2};
        name=[titname ', closest trace to reference'];
    else
        r=thisresult.sref;
        tr=thisresult.tref;
        s=thisresult.seisf(:,thisresult.is1);
        t=thisresult.t;
        name=[titname ', ' thisresult.name ', closest trace to reference'];
    end
    wmethod=getwaveletmethod;
...
elseif(strcmp(action,'alignment'))
    ...
    set(gcf,'name',['Alignment tool: ' name])
...
function [tcc1,tcc2]=getcorrelationgate
    ...
    tt2=get(line2,'ydata');
    tcc2=tt2(1);
    %check for consistency and adjust 
    %if the reflectivity has been changed, then tcc1 may be less than min(tref) and tcc2 could
    %be greater than max(tref). This causes probelms. We check for this and adjust
    if(tcc1<min(tref))
        tcc1=min(tref);
        tt1=[tcc1 tcc1];
        set(line1,'ydata',tt1);
...
if(strcmp(wmethod,'match'))
    if(isempty(MATCHALG) || strcmp(MATCHALG,'matchs'))
        [wlets,tws]=extract_wavelets_match(s(ind),t(ind),sref,t0s,twin,wsize,mum);
    elseif(strcmp(MATCHALG,'matchT'))
        [wlets,tws]=extract_wavelets_matchT(s(ind),t(ind),sref,t0s,twin,wsize,mum);
    else
...
elseif(strcmp(MATCHALG,'matchT'))
    [wlets(4:6),tws(4:6)]=extract_wavelets_matchT(s(ind),t(ind),sref,t0s,twin,wsize,mum);
else
    error('unable to find match algorithm')
end
%[wlets(4:6),tws(4:6)]=extract_wavelets_match(s(ind),t(ind),sref,t0s,twin,wsize,mum);
...
legend([hh{1}(1) hh{2}(1) hh{3}(1)],names,'location','southwest');
title({'wavelets', 'before tying'})
