...
if ~isequal(nfieldnames,nfields)
    error('@Trace/checkDefinition: ''Short Name'' column contains duplicate field names ')
end

%check to see if column1 field names have spaces in them
c = cellfun(@(X) sum(isspace(X)), bd(:,1),'UniformOutput',false);
if sum(cell2mat(c))
   error('@Trace/checkDefinition: ''Short Name'' column contains field names with spaces in them')
end

%check to see if column4 field names have spaces in them
c = cellfun(@(X) sum(isspace(X)), bd(:,1),'UniformOutput',false);
if sum(cell2mat(c))
   error('@Trace/checkDefinition: ''Scalar'' column contains field names with spaces in them')
end
...
if sum(vt) ~= nfields
    ...
    error(['@Trace/checkDefinition: ''Data Type'' column  has invalid datatype(s) at row(s) '...
        br(1:end-1) ])
end
...
if mod(sum(bytes),obj.SIZE)
    ...
    elseif objGUI
        warning(['Total number of bytes represented by ''Data Type'' column is not a multiple of ' ...
            num2str(obj.SIZE)])
    end
end    
...
if sum(vt)
    br = num2str(rownums(vt),'%d, ');
    error(['@Trace/checkDefinition: ''Start Byte'' column has inconsistent startbyte at row(s) '...
        br(1:end-1) ])
end
