function varargout = new ( obj, ntrace, nsamp, sampint, whatsnew )
%
%function varargout = new ( obj, ntrace, nsamp, sampint, whatsnew )
%
...
%    nsamp    = number of sample per trace or [] (default)
%    sampint  = sample interval in seconds or [] (default)
%    segyrev  = 0, 1, 2; default is current object segyrev
...
switch(whatsnew)
    case 'both'
        varargout{1} = obj.newData(ntrace,nsamp);
        varargout{2} = obj.newHeader(ntrace,nsamp,sampint);
    case 'headers'
        varargout{1} = obj.newHeader(ntrace,nsamp,sampint);
        varargout{2} = [];
    case 'data'
        varargout{1} = obj.newData(ntrace,nsamp);
        varargout{2} = [];
