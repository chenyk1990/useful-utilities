...
main (int argc, char **argv)
{
	...
	if (!getparfloat("dxso",&dxso)) err("must specify dxso!\n");
        ...
        if (nxso>1 && dxgo != dxso) err("in this implementation, dxso must = dxgo!\n");
	...
	for(ix=nxf; ix<=nxe; ++ix){
		...
		/* datuming depth at this output location (z) */
                ...
                if(z<=fzo || z>=fzo+(nzo*dzo))                                                    
                        err("Datuming surface is out of travel time range!\n");    
...
/*                                       */
                ...
                if(x>=sx){
                   ...
                   if(hp==0.0) err("Datum and recording surface overlap!\n");
