...
do
	...
	cat wavelet_est.su >> all_shot_wavelets.su
...
##  recever signature
##  find the receiver "wavelet" and deconvolve with sucddecon
##  Sort the data into receiver gathers
rm receiver_gathers.su

susort gx offset < shot_$outfile  > receiver_gathers.su

susplit key=gx  < receiver_gathers.su

# loop over common receiver gather files
for i in `ls split_gx* `
do
	# stack real part of the complex log transform (log|A|)
	suclogfft < $i  | suamp mode=real | sustack key=dt > real.su
	...
	cat wavelet_est.su >> all_receiver_responses.su

	# deconvolve with sucddecon
	sucddecon sufile=wavelet_est.su < $i >> $outfile
done
