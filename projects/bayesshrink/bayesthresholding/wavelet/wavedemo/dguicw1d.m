function varargout = dguicw1d(varargin)
%DGUICW1D Demonstrates continuous 1-D wavelet GUI tools in the Wavelet Toolbox. 
%
% This is a slideshow file for use with wshowdrv.m
% To see it run, type 'wshowdrv dguicw1d', 
%   
%   See also CWT.
...
if nargin>0
	...
	switch action
	  ...
	  case 'getFigParam'
		figName  = 'Wavelet GUI Demo: Continuous Wavelet Analysis';
		showType = 'command';
	  ...
	  case 'slidePROC'
		...
		if isempty(localPARAM)
			active_fig = cw1dtool;
			cw1dmngr('demo',active_fig,'freqbrk','db1',1,1,32,1);
			...
			wshowdrv('#modify_cbClose',figHandle,active_fig,'cw1dtool');
			return
		else
		...
		switch idxSlide
		  ...
		  case 5
			  if idxPREV<idxSlide
				  cw1dmngr('demo',active_fig,'quachirp','db1',1,1,64,1);
				  wenamngr('Inactive',active_fig);
				  map_val = get(pop_pal,'value');
