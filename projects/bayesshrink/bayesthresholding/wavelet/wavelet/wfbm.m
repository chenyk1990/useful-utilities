...
%   $Revision: 1.1.6.2 $  $Date: 2004/03/15 22:42:43 $ 
...
if ~isnumeric(H) | H < 0 | H > 1 | isnan(H)
    error('Fractal index must be a number between 0 and 1');
end
if ~isequal(L,fix(L)) | L < 100 | isnan(L)
    error('Signal length must be an integer greater than 100');
end
if ~isequal(nblev,fix(nblev)) | nblev < 2 | isnan(nblev)
    error('Number of levels must be an integer greater than 1');
end
wtype = wavemngr('type',wname);
if ~isstr(wname) || ~isequal(wtype,1)
    error('Wavelet must be orthogonal');
end
