...
%   $Revision: 1.18.4.2 $ $Date: 2004/03/15 22:40:26 $
...
switch option
    ...
    case 'put'
        ...
        coor = [];
        for i = 1:length(axe_hdls)
            coor = [coor get(axe_hdls(i),'XLim') get(axe_hdls(i),'YLim')];
        end
        fcns = get(Pus_Hist_Init,'UserData');
        %--- empilage de l'etat (fonction utilisateur) ---%
        z1  = eval(fcns(1,:));
        s   = size(z1);
        val = z1(:)';
        DynV_Reg_HPtr = dVTmemB.histPtr;
    ...
    case {'zoom+','zoom-'}
        ...
        if isempty(DynV_Sel_Box)
			...
			wwarndlg(WarnString,'Wavelet Toolbox Zoom','bloc')
			return;
		end
    ...
    case {'Install_V3','Install_V3_CB','create','create_V3'}
        ...
        switch option
            case 'create' % OLD VERSION
                [handles,dynVPos] = CreateDynVTool(fig,varargin{:});
            case {'create_V3','Install_V3','Install_V3_CB'}
                ...
                values = {...
                    ...
                    handles.Edi_Center,     ...
                    handles.Fra_Info,       handles.Fra_Info_Bis,  handles.Txt_Info,  ...
                ...
                fields = {...
                    ...
                    'Edi_Center',  ...
                    'Fra_Info', 'Fra_Info_Bis', 'Txt_Info',  ...
                    'Edi_PosX', 'Edi_PosY'   ...
        ...
        switch option            
            ...
            case 'Install_V3_CB'
                ...
                set(handles.Pus_CenterX    ,'Callback',['dynvtool(''center'',gcbf,' ediStr ',0)'])
                set(handles.Pus_CenterY    ,'Callback',['dynvtool(''center'',gcbf,' ediStr ',1)'])
                set(handles.Pus_CenterY    ,'Callback',['dynvtool(''edi_center'',gcbf,' ediStr ')'])                
                ...
                set(handles.Tog_View_Axes  ,'Callback',[mfilename '(''set_dynvzaxe'',gcbf,gcbo)']);
        end
    ...
    case {'init','attach','go'}
        ...
        %-------------- stockage des parametres de creation --------------%
    ...
    case 'on'
        ...
        if ~isempty(VdynData)
            ...
            [cbox,params]    = wmemutil('get',params);
        else
            continu = 0;
            g_ind = []; g_cmd = []; g_act = []; 
        end
    ...
    case 'stop'
        ...
        dynvfcn = ['mngmbtn(''down'',' strNumFig ');'];
    ...
    case 'fcn_w'
        %***********************************************************%
        %** OPTION = 'fcn_w' CHANGEMENT DE LA FONCTION D'ECRITURE **%
        %***********************************************************%
        dVTmemB       = dynvtool('rmb',fig);
        Fra_DynVTool  = dVTmemB.handles.Fra_DynVTool;
        Edi_PosX = dVTmemB.handles.Edi_PosX;
    ...
    case 'set_BtnOnOff'
		...
		switch typCall
			...
			case 'All'
				...
				switch lower(ena_Val)
				case 'off'
					set(ediCenter,'String','');
					set(infoBtn,'Enable',ena_Val);
			...
			case 'Info'
				...
				switch lower(ena_Val)
					...
					case 'on'  , set(infoBtn,'Enable','Inactive');					
				end			
		end	
...
[Def_TxtBkColor,Def_EdiBkColor,Def_FraBkColor] = ...
    mextglob('get','Def_TxtBkColor','Def_EdiBkColor','Def_FraBkColor');
...
posUIC       = getHorPos(fig,pos,flgTool);
JJ           = 1;
...
comEdiProp = {comFigProp{:},'Style','Edit'};
...
if bitget(flgTool,3)
    ...
    Fra_Info_Bis        = uicontrol(...
        comFraProp{:},                  ...
        'Position',pos_frapos2,         ...
        'BackGroundColor',Def_FraBkColor...
        );
    Edi_PosX   = uicontrol(...
        comEdiProp{:},                  ...
    ...
    Fra_Info = []; Txt_Info = []; Fra_Info_Bis = [];
    Edi_PosX = []; Edi_PosY = [];
end
...
if bitget(flgTool,5)
    figPos = get(fig,'Position');
    graPos = [dynVPos(1),dynVPos(4),dynVPos(3),...
            figPos(4)-dynVPos(4)]./[figPos(3:4),figPos(3:4)];
    ...
    action = ['dynvzaxe(''ini'',' strNumFig ',' num2mstr(graPos) ...
            ',' num2mstr(Tog_View_Axes)  ');'];
    action = [mfilename '(''set_dynvzaxe'',' strNumFig ',' ...
            num2mstr(Tog_View_Axes) ');'];
...
handles = cell2struct(values,fields,2);
%-----------------------------------------------------------------------------%
function posUIC = getHorPos(fig,pos,flags)
...
if okCentTool
    if inBox
        ...
        wfra = wid(2); dy = 0; hedi = haut-bdy/2;
    end
    JJ = JJ+1; posUIC(JJ,:) = [xl,yh-haut,wfra,hautX2];             % pos_fracent
    JJ = JJ+1; 
    posUIC(JJ,:) = [xl+bdx/2,yh-haut+bdy/2,wid(2)-bdx,hautX2-bdy];  % pos_txtcent
    ...
    JJ = JJ+1; posUIC(JJ,:) = [xl,yh-haut+dy,2*wid(3),hedi]; % pos_edcent
...
if okInfoTool
    ...
    xl = xl+wid(4)+bdx/2;
    JJ = JJ+1;
    posUIC(JJ,:) = [xl,yh-haut,wid(5),hautX2];                     % pos_frapos2
    JJ = JJ+1; 
    posUIC(JJ,:) = [xl+bdx/2,yh+bdy/2,wid(5)-bdx,haut-bdy];        % pos_btn_xpos
    JJ = JJ+1; 
    posUIC(JJ,:) = [xl+bdx/2,yh-haut+bdy/2,wid(5)-bdx,haut-bdy];   % pos_btn_ypos
    xl = xl+wid(5)+bdx;
end
