...
%   $Revision: 1.1.6.2 $ $Date: 2004/03/15 22:41:31 $ 
...
if isstr(Regularity)
    ...
    switch Regularity
        ...
        case 'differentiable'
            Regularity = 1;
        otherwise
            error('Invalid value for Regularity.')
    end
else
...
NBContraintes = 1 + 2*(Regularity + 1);
...
switch method
    case 'polynomial' , 
        ...
        %----------------------------------
        if (Regularity > -1) & (N > NBContraintes)
            for i = 1:N
                M([2,3],i) = [a^(i-1) ; b^(i-1)];
            end
            for k = 1:Regularity
            ...
            B(N+2:N+NBContraintes,1) = 0;
        end
        
        % Solving linear system.
        %-----------------------
        A = [G M';M zeros(NBContraintes,NBContraintes)];
        U = A\B;
        PSI = 0;
        for ii = 1:N
    ...
    case 'orthconst' , 
        ...
        if (Regularity > -1) & (N > NBContraintes)
            ...
            MConstraint = zeros(NBContraintes,NBContraintes);
        end
    ...
    case 'symbolic' , 
        ...
        if (Regularity > -1) & (N > NBContraintes)
            ...
            if rk<nbRow
                M([rk+1:nbRow],:) = [];
                NBContraintes = NBContraintes-(nbRow-rk);
            end
            next = size(M,1)+1;
            if Regularity > 0
                for i = 1:N
        ...
        if NBContraintes>0 , B(N+1:N+NBContraintes,1) = 0; end
