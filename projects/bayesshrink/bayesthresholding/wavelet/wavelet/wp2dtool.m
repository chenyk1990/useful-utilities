...
%   $Revision: 1.20 $ $Date: 2002/06/17 12:18:54 $
    ...
    case 'create'
        ...
        [win_wptool,pos_win,win_units,str_numwin,...
             frame0,pos_frame0,Pos_Graphic_Area,pus_close] = ...
                 wfigmngr('create','Wavelet Packets 2-D',winAttrb,'ExtFig_Tool',...
                               mfilename,1,1,0);
		...
		% Add Help for Tool.
		%------------------
		wfighelp('addHelpTool',win_wptool,'T&wo-Dimensional Analysis','WP2D_GUI');
        ...
        y_low           = y_low-Def_Btn_Height-dy;
        w_base          = cmd_width-2*dx2-dx;
        pos_t_curtree   = [x_left, y_low, (2*w_base)/3, Def_Btn_Height];
        pos_curtree     = pos_t_curtree;
        pos_curtree(1)  = pos_t_curtree(1)+pos_t_curtree(3)+dx;
        pos_curtree(3)  = w_base/3;
        ...
        yl              = y_low-Def_Btn_Height/2;
        pos_txt_full    = [x_left, yl, w_base/3,Def_Btn_Height];

        xl                = pos_txt_full(1)+pos_txt_full(3)+dx;
        pos_pus_full      = zeros(4,4);
        pos_pus_full(1,:) = [xl, y_low, w_base/3, Def_Btn_Height];
        ...
        %------------------------------
        cba_WPOpt      = 'wptreeop';
        cba_anal       = ['wp2dmngr(''anal'',' str_numwin ');'];
        cba_comp       = ['wp2dmngr(''comp'',' str_numwin ');'];
        cba_deno       = ['wp2dmngr(''deno'',' str_numwin ');'];
